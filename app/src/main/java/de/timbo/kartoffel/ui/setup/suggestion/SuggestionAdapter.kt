...
class SuggestionAdapter(private val windowManager: WindowManager) : RecyclerView.Adapter<SuggestionAdapter.SelectViewHolder>() {
    ...
    inner class SelectViewHolder(private val ib: SwipeListItemBinding) : RecyclerView.ViewHolder(ib.root) {
        fun bind(recipes: List<Recipe>) {
            ...
            val recyclerView = flipView.findViewById<RecyclerView>(R.id.swipe_list_item_front_Rv)
            recyclerView.adapter = RecipesAdapter(recipes)
            val displayMetrics = DisplayMetrics()
            windowManager.defaultDisplay.getMetrics(displayMetrics)

            val a = displayMetrics.heightPixels * 90 / 100
